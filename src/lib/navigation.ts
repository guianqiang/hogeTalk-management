...
const platformNavigation: ManagementNavGroup[] = [
  ...
  },
  {
    label: '网站内容',
    items: [
      { href: '/legacy/home', label: '首页管理', note: '管理首页内容', icon: 'content', capability: 'dashboard.read' },
  ...
  },
  {
    label: '组织与撮合',
    items: [
      { href: '/enterprises', label: '企业管理', note: '企业主体与认证状态', icon: 'enterprise', capability: 'dashboard.read' },
      { href: '/legacy/chambers', label: '商会管理', note: '管理商会主体', icon: 'membership', capability: 'dashboard.read' },
  ...
  },
  {
    label: '审核与治理',
    items: [
      { href: '/verifications', label: '平台认证', note: '审核企业 L1–L3 平台认证', icon: 'verification', capability: 'enterprise.verify' },
      { href: '/claims', label: '认领审核', note: '处理企业认领申请', icon: 'review', capability: 'claim.review' },
      { href: '/duplicates', label: '重复企业', note: '核查疑似重复主体', icon: 'duplicate', capability: 'duplicate.review' },
      { href: '/disputes', label: '所有权争议', note: '处理企业控制权争议', icon: 'dispute', capability: 'dispute.review' },
    ],
  },
  {
    label: '站点与字典',
    items: [
      { href: '/legacy/partners', label: '合作伙伴', note: '管理合作伙伴', icon: 'partner', capability: 'dashboard.read' },
      ...
      { href: '/legacy/countries', label: '国家管理', note: '管理国家与地区', icon: 'globe', capability: 'dashboard.read' },
  ...
  },
  {
    label: '系统',
    items: [
      { href: '/account-members', label: '账号管理', note: '管理后台人员和授权范围', icon: 'account', capability: 'staff.manage' },
...
const chamberNavigation: ManagementNavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/account-members', label: '账号管理', note: '管理商会后台账号', icon: 'account', capability: 'staff.manage' },
...
export function navigationForWorkspace(workspace: Workspace): ManagementNavGroup[] {
  ...
  return groups.flatMap((group) => {
    const items = group.items.filter((item) => workspace.capabilities.includes(item.capability))
